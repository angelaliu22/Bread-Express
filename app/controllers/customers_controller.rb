class CustomersController < ApplicationController
  ...
  def show
        @customer_pending_orders = Order.for_customer(@customer.id).not_shipped.chronological.paginate(:page => params[:page]).per_page(5)
        @customer_past_orders = Order.for_customer(@customer.id).shipped.chronological.paginate(:page => params[:page]).per_page(5)
  end
